...
class European:
	...
	def MonteCarlo(self, option):
		if option.lower() == "call":
			simulations = 50000
			disc_factor = math.exp(-self.r * self.T)
			payoffs = []
			for i in xrange(simulations):
				S_t = self.S * math.exp((self.r - 0.5 * self.sigma**2) * self.T + self.sigma * math.sqrt(self.T) * gauss(0,1.0))
	...
	def BlackScholes(self, option):
		d1 = ((self.S/self.X) + (self.r-self.d + ((self.sigma**2)/2))) / (self.sigma*math.sqrt(self.T))
		d2 = d1 - self.sigma*math.sqrt(self.T)
		if option.lower() == "call":
			return self.S * math.exp(-self.d*self.T) * norm.cdf(d1) - self.X * math.exp(-self.r*self.T) * norm.cdf(d2)
		elif option.lower() == "put":
			return self.X * math.exp(-self.r*self.T) * norm.cdf(-d2) - self.S * math.exp(-self.d*self.T) * norm.cdf(-d1)
		else:
			return "Invalid data"
		#return price
